...
generator = torch.nn.parallel.DistributedDataParallel(generator, device_ids=[opt.gpu])
opt.batch_size = opt.batch_size // len(gpus)

if hr_shape == 256:
    ie = InceptionExtractor256()
    discriminator = Discriminator256()
    '''ie = torch.nn.DataParallel(InceptionExtractor256(), device_ids=gpus)#.to(device)
    discriminator = torch.nn.DataParallel(discriminator, device_ids=gpus)#.to(device)'''
    ie, discriminator = ie.cuda(), discriminator.cuda()
    ie = torch.nn.parallel.DistributedDataParallel(ie, device_ids=[opt.gpu])
    discriminator = torch.nn.parallel.DistributedDataParallel(discriminator, device_ids=[opt.gpu])
...
train_data=ImageDataset("%s" % opt.dataset_name, hr_shape=hr_shape, ratio=opt.mask_ratio)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    while imgs != None:
        ...
        loss_G = opt.lambda_adv * loss_GAN + prep_loss
        ...
        if i%20 == 0:
            print('[%d][%d/%d], G-Loss:[%f], D-Loss:[%f], Adv-Loss:[%f], Prep-Loss:[%f]'%(epoch, i, total_batch, loss_G.item(), loss_D.item(), loss_GAN.item(), prep_loss.item()))        
