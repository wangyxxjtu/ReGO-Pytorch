...
parser.add_argument("--sk_mask_rate", type=float, default=0.0, help="mask the sketch rate")
...
for i in trange(len(all_images)):
        ...
        sk_mask = torch.ones_like(sk_tensor)
        sk_mask[:,:,:,128:(128+int(128*opt.sk_mask_rate))] = 0
        sk_tensor = sk_mask * sk_tensor

        cand_list = im_cands[all_images[i]] 
        
        if 'test' in opt.image_path:
            #ref = torch.zeros_like(sk_tensor).to(device)
            #ref = ref.repeat(1,3,1,1)
            ref = torch.unsqueeze(img, 0)
        else: 
        ...
        with torch.no_grad():
            for k in range(len(cand_list)):
                    ...
                    if opt.rand_test:
                        ...
                        cand = all_candidates[rand]
                        sk_ref_img = Image.open(cand).convert('RGB').resize((256, 128))
                        sk_ref_img= np.asarray(sk_ref_img).astype("f").transpose(2, 0, 1) / 127.5 - 1.0
                        sk_ref_img = torch.from_numpy(sk_ref_img).to(device).unsqueeze(0)
                    #else:
                    #prepare the detail guide sketch
                    sk_name = cand.replace('im_', 'sk_')
                    if opt.rand_test:
                        sk_pth = os.path.join(sk_name)
                    else:
                        sk_pth = os.path.join(opt.base_path, sk_name)
                    sk = Image.open(sk_pth).resize((256, 128))
                    sk_arr = np.asarray(sk).astype("f") / 255.0

                    sk_arr = np.where(sk_arr>=0.6, 1, 0)
                    sk_Detail_t = torch.tensor(sk_arr)
                    sk_Detail_t = sk_Detail_t.float()
                    sk_Detail_t = sk_Detail_t.unsqueeze(0).to(device)
                    sk_Detail_t = sk_Detail_t.unsqueeze(1)
                    sk_tensor = torch.cat([sk_tensor[:,:,:,:128], sk_Detail_t[:,:,:,128:]], dim=-1)
