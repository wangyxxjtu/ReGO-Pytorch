...
def calculate_activation_statistics(images, sess, batch_size=50, verbose=False):
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the incption model.
    """
    act = get_activations(images, sess, 1, verbose)
...
def _handle_path(path, sess, low_profile=False, gen_mode=''):
    if path.endswith('.npz'):
        ...
        if gen_mode == '':
            print('raw image num:', len(files))
        else:
            print('generate mode: %s, num: %d'%(gen_mode, len(files)))
        if low_profile:
...
def calculate_fid_given_paths(paths, inception_path, low_profile=False, gen_mode='m0', gpu='2'):
    ...
    with tf.Session() as sess:
        ...
        m2, s2 = _handle_path(paths[1], sess, low_profile=low_profile, gen_mode=gen_mode)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gen_mode", type=str, default='')
    
    args = parser.parse_args()
    os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    fid_value = calculate_fid_given_paths(args.path, args.inception, low_profile=args.lowprofile, gen_mode=args.gen_mode)
    print("FID: ", fid_value)
